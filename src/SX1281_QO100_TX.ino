...
#define TIMER_FREQUENCY 400     // 1/2.5 msec=400
...
uint8_t wpm_delay_and_paddle_check (uint32_t delay_ms, uint8_t keyerReleaseMask2, uint8_t return_val_when_released) {
  ...
  for (uint32_t d = 0; d < delay_ms; d++) {
    ...
    if (RotaryEnc_KeyerType.cntVal == 2) {  // if Iambic-B keyer is configured (0 = Iambic-A, 1 = Straight, 2 = Iambic-B)
      ...
      if ((keyerVal == 0x03) || (keyerVal == keyerReleaseMask2)) {
        return_val = return_val_when_released; // if during dot/dash playing both keyers released then play additional dash/dot
      }
    }
  }
  return return_val;
...
void setup() {
  ...
  ledcAttachChannel(BUZZER, 8000, 8, 3);
  ...
  timer = timerBegin(1000000);  // Fréquence en Hz
  timerAttachInterrupt(timer, onTimer);
  timerAlarm(timer, TIMER_PERIOD_USEC,true,0);
  //timerWrite(timer, TIMER_PERIOD_USEC);
  //timerStart(timer);
